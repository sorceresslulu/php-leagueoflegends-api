...
$service = new LolAPI\Service\League\Ver2_5\ByTeamIdsEntry\Service(
    $apiHandler,
    $leagueQueueTypeFactory,
    $leagueTierFactory
);

$request = new LolAPI\Service\League\Ver2_5\ByTeamIdsEntry\Request(
    $apiKey, $region, array($config['teamId'])
);
$query = $service->createQuery($request);
$queryResult = $query->execute();

function processQueryResult(LolAPI\Service\League\Ver2_5\ByTeamIdsEntry\QueryResult $queryResult) {
