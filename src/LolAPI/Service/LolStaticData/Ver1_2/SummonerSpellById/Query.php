...
class Query
{
    const QUERY_TYPE = 'lol-static-data-ver1.2-summoner-spell-by-id';
    ...
    /**
     * LolStaticData.SummonerSpellById request
    ...
    public function execute()
    {
        ...
        if ($request->getRegionalEndpoint()->hasRegionCode()) {
            $serviceUrl = sprintf(
                'https://%s/api/lol/static-data/%s/v1.2/summoner-spell/%d',
                $globalEndpoint->getHost(),
                strtolower($request->getRegionalEndpoint()->getRegionCode()),
                $request->getSummonerSpellId()
            );
        } else {
