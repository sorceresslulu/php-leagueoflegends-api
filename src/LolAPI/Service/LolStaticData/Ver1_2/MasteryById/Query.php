...
use LolAPI\Exceptions\MasteryNotFound;
...
class Query
{
    const QUERY_TYPE = 'lol-status-ver1.0-mastery-by-id';
    ...
    /**
     * LolStaticData.MasteryById query
    ...
    public function execute()
    {
        ...
        if($request->isMasteryListDataSpecified()) {
            $urlParams['masteryListData'] = implode(',', $request->getMasteryListData());
        }

        if ($request->getRegionalEndpoint()->hasRegionCode()) {
            $serviceUrl = sprintf(
                'https://%s/api/lol/static-data/%s/v1.2/mastery/%d',
                $globalEndpoint->getHost(),
                strtolower($request->getRegionalEndpoint()->getRegionCode()),
                $request->getMasteryId()
            );
        } else {
        ...
        if($response->isSuccessful()) {
            ...
            switch($response->getHttpCode()) {
                ...
                case 404: throw new MasteryNotFound($response);
